...
const giveawayInteractionTimestamps = new Map();
...
export function isUserRateLimited(userId, giveawayId) { const key = getGiveawayInteractionKey(userId, giveawayId); const last = giveawayInteractionTimestamps.get(key) || 0; return Date.now() - last < GIVEAWAY_INTERACTION_COOLDOWN; }
export async function recordUserInteraction(userId, giveawayId) { const key = getGiveawayInteractionKey(userId, giveawayId); giveawayInteractionTimestamps.set(key, Date.now()); setTimeout(() => giveawayInteractionTimestamps.delete(key), GIVEAWAY_INTERACTION_COOLDOWN + 1000).unref?.(); }
