...
async function calculateModalHandler(interaction, client, args) {
    try {
        ...
        const operandInput = modalFields?.fields?.first?.();
        ...
        const { expression, operator, rootKey } = context;
        ...
        if (rootKey) calculationContexts.set(rootKey, { expression: newExpression, formattedResult: formattedNewResult, userId: context.userId });
